...
X <- subset(df, select = -c(WINS))
y <- df$WINS
X_pca = prcomp(X, rank. = 12)$x
```

```{r}
data = as.data.frame(cbind(X_pca, y))
head(data)
```
...
```{r}
library(sgd)
sgd.theta <- sgd( y ~ ., data = data, model="lm")
sgd.theta
```
...
```{r}
be_lm1_1 <- lm(WINS ~ bt_H + bt_SO + br_SB + ph_HR + fd_E + fd_DP + ph_BB, data = df)
summary(be_lm1_1)
```
